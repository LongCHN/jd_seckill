...
class Timer(object):
    def __init__(self, sleep_interval=0.5):
        ...
        self.buy_time = datetime.strptime(
            '%(day)s %(hour)s' % dict(day=today, hour=global_config.getRaw('config', 'buy_time')),
            "%Y-%m-%d %H:%M:%S.%f")
        # 抢5分钟就不抢了
    ...
    def is_time_over(self):
        return self.real_time() > self.last_buy_time_ms
...
class ReserveTimer(Timer):
    def is_time_ready(self):
        return self.real_time() >= self.before_buy_time_ms


class BuyTimer(Timer):
    def is_time_ready(self):
        return self.real_time() >= self.buy_time_ms
